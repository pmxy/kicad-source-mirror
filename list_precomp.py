...
# Sorts the files by number of inclusions, rather than by
# "score" = nuber of inclusion * (inclusion depth)
# 
...
def parse_include (indent, inc, ignore_path, current_info, include_map):
    ...
    if include_map.has_key(inc):
        ...
        rp ="#include \"" + inc + "\""
#        if not (rp.startswith("/usr") or rp.startswith("/home")) :
        print rp
...
#        m = include_regexp_gcc.search(line)
        ...
        if m:
            ...
            inc =  os.path.realpath(cleanup_path(m.group(2)))
